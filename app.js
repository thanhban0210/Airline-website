...
app.get('/select-table', async (req, res) => {
    ...
    try {
        ...
        if (type === 'insert') {
            ...
            try {
                let table4 = await client.query(`SELECT * FROM ${table_name}`)
...
const translateError = (e) => {
    if (e) {
        // if (e.startsWith("duplicate key value violates unique constraint")) {
        //     if (table_name == "airports") {
        //         e = "Error: The airport you are trying to add already exists."
        //     }
        //     else if (table_name == "flights") {
        //         e = "Error: The flight you are trying to add already exists."
        //     }
        //     else if (table_name == "aircrafts") {
        //         e = "Error: The aircraft you are trying to add already exists."
        //     } else {
        //         e = "Error: Duplicate primary key."
        //     }
        // }
        return e
    }
...
app.post('/insert_flight', async (req, res) => {
    ...
    try {
        ...
        if (dGateViolation.rows[0].violation == true) { /*shared DEPARTURE gate*/
                ...
                if (newArrGateViolation.rows[0].violation == true) {/*new arrival gate is busy*/
                    ...
                    if (newDepGateViolation.rows[0].violation == true) {/*new departure gate is busy*/
                        ...
                        await client.query(`BEGIN TRANSACTION;
                                            ...
                                            COMMIT;`)
                        try {
                            let table = await client.query(`SELECT * FROM FLIGHTS`)
...
app.post('/insert_airport', async (req, res) => {
    ...
    try {
        let table = await client.query(`SELECT * FROM AIRPORTS`)
...
app.post('/insert_aircraft', async (req, res) => {
    ...
    try {
        //Thien
        // var no_economy = input.no_economy_seats;
        // var no_business = input.no_business_seats;
        // var no_first_class = input.no_first_class_seats;
        // await client.query(`
        // do $$
        // declare
        // numEco int := ${no_economy};
        // numBus int := ${no_business};
        // numFir int := ${no_first_class};
    ...
    try {
        let table = await client.query(`SELECT * FROM AIRcrafts`)
            ...
            if (aGateViolation.rows[0].violation == true) { /*shared ARRIVAL gate */
                ...
                if (newArrGateViolation.rows[0].violation == true) {/*new arrival gate is busy*/
                    ...
                    if (newDepGateViolation.rows[0].violation == true) {/*new departure gate is busy*/
                        ...
                        await client.query(`BEGIN TRANSACTION;
                        UPDATE flights
                        SET flight_id = '${input.flight_id}', scheduled_departure = TO_TIMESTAMP('${input.departure_time}','YYYY-MM-DD HH24:MI'), scheduled_arrival = TO_TIMESTAMP('${input.arrival_time}','YYYY-MM-DD HH24:MI'), departure_airport = '${input.departure_airport}', arrival_airport = '${input.arrival_airport}', departure_gate = '${input.departure_gate}', arrival_gate = '${input.arrival_gate}', aircraft_code = '${input.aircraft_code}', refueling = '${input.refuel.toUpperCase()}', cleaning = '${input.cleaning.toUpperCase()}',maintenance = '${input.maintainance.toUpperCase()}');
                    COMMIT;`)
                    }
                    try {
                        let table = await client.query(`SELECT * FROM FLIGHTS`)
...
app.post('/sql_update_aircraft', async (req, res) => {
    ...
    try {
        let table = await client.query(`BEGIN TRANSACTION;
        UPDATE Aircrafts
    ...
    try {
        let table = client.query(`SELECT * FROM AIRcrafts`)
